...
def run_klee_on_file(filepath, logDir, outputdir, useDirName=False):
    ...
    try:
        ...
        with open(log_file, 'w') as output_file:
            subprocess.run(['CUDAAnalyzer', f"--timeout={one_timeout}", f"--output-dir={outputdir}", filepath], stdout=output_file, stderr=output_file, timeout=TIMEOUT_LIMIT, check=True)
        ...
        with open(log_file, 'r') as output_file:
            ...
            if "KLEE: done: completed paths =" not in log_content:
                ...
                print(f"Successfully ran KLEE on {filepath}. Output saved to {log_file}")
                return True
...
if __name__ == "__main__":
    ...
    if args.bc_dir:
        ...
        log_dir = "./logs"
        os.makedirs(log_dir, exist_ok=True)
        main_multiple_threads(args.bc, log_dir, out_dir, False, max_threads)
        ...
        log_dir = "./logs"
        os.makedirs(log_dir, exist_ok=True)
        main_multiple_threads(args.json, log_dir, out_dir, True, max_threads)
