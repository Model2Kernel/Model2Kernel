...
namespace {
  ...
  cl::opt<std::string>
  CudaType("cuda-type",
             cl::desc("entry function type of cuda program (default=OFF)"),
             cl::init("OFF"),
             cl::cat(StartCat));
  ...
  cl::opt<std::string>
  FuncConfig("func-config",
            cl::desc("Function config such as tensor shape"),
            cl::init(""),
            cl::cat(StartCat));
            
  cl::opt<int>
  JsonIndex("jindex",
            cl::desc("index of json array"),
            cl::init(-1),
            cl::cat(StartCat));
...
public:
  KleeHandler(int argc, char **argv, std::string functionName);
  ~KleeHandler();
...
KleeHandler::KleeHandler(int argc, char **argv, const std::string functionName)
    : m_interpreter(0), m_pathWriter(0), m_symPathWriter(0),
      m_outputDirectory(), m_numTotalTests(0), m_numGeneratedTests(0),
      m_pathsCompleted(0), m_pathsExplored(0), m_argc(argc), m_argv(argv), m_functionName(functionName) {
  ...
  // } else {
    ...
    for (; i < INT_MAX; ++i) {
      ...
      std::string tmp = std::string("klee-out-");
      if (JsonIndex>=0) {
        tmp += "jindex-" + std::to_string(JsonIndex) + "-";
      }
      llvm::sys::path::append(d, tmp);
      ...
      if (mkdir(d.c_str(), 0775) == 0) {
        ...
        klee_error("index %d has been executed.", JsonIndex.getValue());
      }
...
void KleeHandler::setInterpreter(Interpreter *i) {
  ...
  m_interpreter->setOutputDir(m_outputDirectory.c_str());
}
...
int main(int argc, char **argv, char **envp) {
  ...
  if (const llvm::NamedMDNode *nvvmAnnotations = mainModule->getNamedMetadata("nvvm.annotations")) {
  ...
  if (CudaType.empty())
    klee_error("cuda-type cannot be empty");
  
  if (CudaType != "OFF" && CudaType != "kernel" && CudaType != "caller") {
    klee_error("cuda-type not support");
  }

  bool isCuda = CudaType == "kernel" || CudaType == "caller";

  std::string LibraryDir = KleeHandler::getRunTimeLibraryPath(argv[0]);
  Interpreter::ModuleOptions Opts(LibraryDir.c_str(), EntryPoint, opt_suffix,
                                  /*Optimize=*/OptimizeModule,
                                  /*CheckDivZero=*/CheckDivZero,
                                  /*CheckOvershift=*/CheckOvershift, isCuda, CudaType == "caller");
  ...
  std::string fName = isCuda ? EntryPoint.getValue() : "";
  Interpreter::InterpreterOptions IOpts;
  IOpts.MakeConcreteSymbolic = MakeConcreteSymbolic;
  KleeHandler *handler = new KleeHandler(pArgc, pArgv, fName);
  ...
  interpreter->setMainModule(mainModule);
  interpreter->setKernelNames(kernelFunctionNames);

  if (!FuncConfig.getValue().empty()) {
    std::ifstream file(FuncConfig);
    if (!file.is_open()) {
      llvm::errs() << "Failed to open JSON file.\n";
    } else {
      nlohmann::json j;
      file >> j;

      std::map<unsigned, Interpreter::ParameterValue> argumentValues;
      std::map<std::string, std::vector<std::tuple<int, int, int>>> equals; // -1,index,j: max, -2,index,j: min
      std::map<std::string, std::pair<int, int>> symRanges;

      if (j.is_array()) {
        auto& call = j[JsonIndex];
        bool isFirstSRet = entryFn->hasParamAttribute(0, llvm::Attribute::StructRet);

        std::map<std::string, unsigned> promptCons;
        if (call.contains("batch_size")) {
          const auto& value = call["batch_size"];
          if (value.is_number()) {
        ...
        if (call.contains("args") && call["args"].is_array()) {
          for (size_t i = 0; i < call["args"].size(); ++i) {
            ...
            if (arg.contains("value") && arg["value"].is_array()) {
              int len = arg["value"].size();
              std::string dtype = "";
              if (arg.contains("dtype")) {
                dtype = arg["dtype"];
              }
              ...
              for (int m = 0; m < len; ++m) {
                ...
                if (item.contains("symRanges")) {
                  for (auto it = item["symRanges"].begin(); it != item["symRanges"].end(); ++it) {
                    std::string expr = it.key();
                    int symMin = it.value()[0];
                    int symMax = it.value()[1];
                    if (symRanges.find(expr)!=symRanges.end()) {
                ...
                if (item.contains("maxV")) {
                  if (item["maxV"].is_number()) {
                    maxVal = item["maxV"].get<int>();
                  } else {
                ...
                if (item.contains("minV")) {
                  if (item["minV"].is_number()) {
                    minVal = item["minV"].get<int>();
                  } else {
                ...
                if (maxVal == minVal && maxVal == 0) {
                  maxVal = INT32_MAX;
                  minVal = INT32_MIN;
                } else if (item.contains("dupV")) {
                  hasDuplicateVal = item["dupV"].get<bool>();
                }
                Interpreter::ParameterValue elp(elType, "", shapeSize, elDtype, maxVal, minVal, hasDuplicateVal);
                elp.shape = shape;
                content.push_back(elp);
              }
              ...
              if (arg.contains("symRanges")) {
                for (auto it = arg["symRanges"].begin(); it != arg["symRanges"].end(); ++it) {
                  std::string expr = it.key();
                  int symMin = it.value()[0];
                  int symMax = it.value()[1];
                  if (symRanges.find(expr)!=symRanges.end()) {
              ...
              if (maxVal == minVal && maxVal == 0) {
                maxVal = INT32_MAX;
                minVal = INT32_MIN;
              } else if (arg.contains("dupV")) {
                hasDuplicateVal = arg["dupV"].get<bool>();
              }
  ...
  if (!ReplayKTestDir.empty() || !ReplayKTestFile.empty()) {
    ...
    for (std::vector<KTest*>::iterator
      ...
      if (isCuda) {
        interpreter->runKernelFunction(entryFn, Opts.hasLaunchKernel);
      } else
