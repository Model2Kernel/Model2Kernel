...
bool TimingSolver::mustBeFalse(const ConstraintSet &constraints, ref<Expr> expr,
                               bool &result, SolverQueryMetaData &metaData, std::map<std::string, unsigned> intArrNames, std::vector<ref<Expr>> noCheckExprs) {
  return mustBeTrue(constraints, Expr::createIsZero(expr), result, metaData, intArrNames);
}

bool TimingSolver::mayBeTrue(const ConstraintSet &constraints, ref<Expr> expr,
                             bool &result, SolverQueryMetaData &metaData, std::map<std::string, unsigned> intArrNames, std::vector<ref<Expr>> noCheckExprs) {
  bool res;
  if (!mustBeFalse(constraints, expr, res, metaData, intArrNames, noCheckExprs))
    return false;
  result = !res;
  return true;
}
...
bool TimingSolver::mayBeFalse(const ConstraintSet &constraints, ref<Expr> expr,
                              bool &result, SolverQueryMetaData &metaData, std::map<std::string, unsigned> intArrNames, std::vector<ref<Expr>> noCheckExprs) {
  bool res;
  if (!mustBeTrue(constraints, expr, res, metaData, intArrNames, noCheckExprs))
    return false;
  result = !res;
  return true;
}
...
bool TimingSolver::getValue(const ConstraintSet &constraints, ref<Expr> expr,
                            ...
                            SolverQueryMetaData &metaData, std::map<std::string, unsigned> intArrNames, std::vector<ref<Expr>> noCheckExprs) {
  ++stats::queries;
  // Fast path, to avoid timer and OS overhead.
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(expr)) {
    result = CE;
  ...
  bool success = solver->getValue(Query(constraints, expr, intArrNames, noCheckExprs), result);
...
bool TimingSolver::getInitialValues(
  ...
  bool success = solver->getInitialValues(
      Query(constraints, ConstantExpr::alloc(0, Expr::Bool), intArrNames, noCheckExprs), objects, result);
...
TimingSolver::getRange(const ConstraintSet &constraints, ref<Expr> expr,
  ...
  ++stats::queries;
  TimerStatIncrementer timer(stats::solverTime);
  auto result = solver->getRange(Query(constraints, expr, intArrNames, noCheckExprs));
  metaData.queryCost += timer.delta();
  return result;
}